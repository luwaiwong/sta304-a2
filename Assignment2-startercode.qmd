...
ggplot(phone_data, aes(x = gender)) +
  geom_bar(fill = "steelblue") +
  labs(title = "Distribution of Gender in Phone Survey Data",
       x = "Gender",
       y = "Count") +
  theme_minimal()
...
To estimate the proportion of individuals voting for the Liberal party, we calculate a 95% confidence interval for the proportion. The general formula for a confidence interval for a proportion is:(UPDATE! NEED CONFIRM)

$$\hat{p} \pm z_{\alpha/2} \cdot \sqrt{\frac{\hat p(1-\hat p)}{n}}$$

Where:

-   $\hat X$ is the sample proportion of individuals who voted for the Liberal party.
-   $\pm z_{\alpha/2}$​ is the critical value from the standard normal distribution corresponding to the desired confidence level (for a 95% confidence interval, $\pm z_{\alpha/2}=1.96$).
-   $n$ is the sample size of the survey respondents with valid vote choice data.
-   $\sqrt{\frac{\hat p(1-\hat p)}{n}}$ is the standard error of the proportion of individuals who voted for the Liberal party

Our logistic regression model for predicting the log odds of voting for the Liberal party is described mathematically as below (**TODO REVIEW**):

$$log(\frac{p}{1-p}) = \beta_0 + \beta_1 x_{male} + \beta_2x_{female} + \beta_3x_{age}$$

Independent variables for the model are:

-   Male: A categorical variable which is 0 if the voter is not male, 1 if the voter is male. Describes the log odds of someone voting Liberal, if they are male

-   Female: A categorical variable which is 0 if the voter is not female, 1 if the voter is female. Describes the log odds of someone voting Liberal, if they are female
